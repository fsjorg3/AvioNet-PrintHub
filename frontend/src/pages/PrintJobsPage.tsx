...
export function PrintJobsPage() {
  ...
  return <Stack spacing={3}><Box><Typography variant="h4">Trabajos de impresión</Typography><Typography color="text.secondary">Historial reportado por los kioscos.</Typography></Box><TextField label="Filtrar por PIN" value={pin} onChange={event => { setPin(event.target.value); setModel(current => ({ ...current, page: 0 })); }} sx={{ maxWidth: 260 }} /><DataGrid autoHeight rows={query.data!.items} columns={columns} loading={query.isFetching} paginationMode="server" rowCount={query.data!.pagination.total} paginationModel={model} onPaginationModelChange={setModel} pageSizeOptions={[10, 25, 50]} disableRowSelectionOnClick onRowClick={({ row }) => navigate(`/print-jobs/${row.id}`)} sx={{ '& .MuiDataGrid-row': { cursor: 'pointer' } }} /></Stack>;
}
