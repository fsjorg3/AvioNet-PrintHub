...
export function DashboardPage() {
  ...
  return <Stack spacing={3}><Box><Typography variant="h4">Dashboard</Typography><Typography color="text.secondary">Resumen operativo de AvioNet PrintHub.</Typography></Box>
    ...
    <Grid container spacing={2}><Grid size={{ xs: 12, lg: 7 }}><Paper sx={{ p: 2 }}><Typography variant="h6" gutterBottom>Ingresos por kiosco</Typography>{data.byKiosk.length ? <BarChart height={320} xAxis={[{ data: data.byKiosk.map(item => item.name), scaleType: 'band' }]} series={[{ data: data.byKiosk.map(item => item.revenue), label: 'Ingresos (MXN)', color: '#FF6B00' }]} /> : <Typography color="text.secondary">Aún no hay trabajos registrados.</Typography>}</Paper></Grid>
      <Grid size={{ xs: 12, lg: 5 }}><Paper sx={{ p: 2 }}><Typography variant="h6" gutterBottom>Consumibles actuales</Typography><Stack spacing={1}>{consumables.data!.consumables.length ? consumables.data!.consumables.map(item => <Box key={`${item.kiosk_id}-${item.type}`} sx={{ display: 'flex', justifyContent: 'space-between', gap: 1 }}><Typography>{item.kiosk_id} · {item.type}</Typography><Chip size="small" label={item.status} color={item.status === 'ok' ? 'success' : item.status === 'low' ? 'warning' : item.status === 'empty' || item.status === 'critical' ? 'error' : 'default'} /></Box>) : <Typography color="text.secondary">Sin reportes de consumibles.</Typography>}</Stack></Paper></Grid></Grid>
  </Stack>;
}
