...
export function PrintJobDetailPage() {
  ...
  return <Stack spacing={3}><Button component={Link} to="/print-jobs" sx={{ alignSelf: 'start' }}>← Volver al historial</Button><Typography variant="h4">Trabajo #{item.id}</Typography><Paper sx={{ p: 3 }}><Stack spacing={1}><Typography><strong>Kiosco:</strong> {item.kiosk_name}</Typography><Typography><strong>PIN:</strong> {item.pin || 'Manual'}</Typography><Typography><strong>Páginas:</strong> {item.pages}</Typography><Typography><strong>Ingreso:</strong> ${item.revenue.toFixed(2)}</Typography><Typography><strong>Fecha:</strong> {item.created_at}</Typography></Stack></Paper></Stack>;
}
