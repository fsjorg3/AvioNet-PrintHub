...
export function ConsumablesPage() {
  ...
  return <Stack spacing={3}><Box><Typography variant="h4">Consumibles</Typography><Typography color="text.secondary">Último reporte por tipo y kiosco.</Typography></Box><Box sx={{ width: '100%', minWidth: 0 }}><DataGrid autoHeight rows={query.data!.consumables} columns={columns} getRowId={row => `${row.kiosk_id}-${row.type}`} disableRowSelectionOnClick pageSizeOptions={[10, 25, 50]} initialState={{ pagination: { paginationModel: { pageSize: 25 } } }} /></Box></Stack>;
}
