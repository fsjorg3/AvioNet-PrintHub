...
export function KiosksPage() {
  ...
  return <Stack spacing={3}><Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><Box><Typography variant="h4">Kioscos</Typography><Typography color="text.secondary">Administra precios y disponibilidad.</Typography></Box><Button variant="contained" startIcon={<Add />} onClick={() => setDialog(true)}>Nuevo kiosco</Button></Box>
    <DataGrid rows={kiosks.data!.kiosks} columns={columns} getRowId={row => row.id} autoHeight disableRowSelectionOnClick pageSizeOptions={[10, 25, 50]} initialState={{ pagination: { paginationModel: { pageSize: 10 } } }} />
